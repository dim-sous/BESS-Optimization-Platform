"""Nonlinear tracking MPC with temperature constraints and control blocking.

v2_thermal_model: adds temperature as a predicted state with soft constraints.
...
---------
  min  sum_k  Q_soc (SOC_k - ref)^2  +  Q_soh (SOH_k - ref)^2
     ...
     + slack_penalty * sum eps_soc_k^2
     + slack_penalty_temp * sum eps_temp_k^2

Soft constraints on SOC and Temperature; hard bounds on SOH and power.
"""
...
from config.parameters import BatteryParams, MPCParams, ThermalParams, TimeParams
...
class TrackingMPC:
    ...
    def _build_problem(self) -> None:
        ...
        F_mpc = build_casadi_rk4_integrator(bp, thp, self.tp.dt_mpc)
    ...
    def solve(
        ...
        Parameters
        ----------
        x_est     : ndarray (3,)   [SOC_est, SOH_est, T_est]
        soc_ref   : ndarray (N+1,)
        soh_ref   : ndarray (N+1,)
