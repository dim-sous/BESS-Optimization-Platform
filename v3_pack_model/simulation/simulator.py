"""Multi-rate simulation coordinator with multi-cell pack model.

v3_pack_model: replaces single-cell BatteryPlant with BatteryPack.
EMS / MPC / EKF / MHE remain pack-level (3-state, unchanged from v2).
...
# ---------------------------------------------------------------------------
#  Reference interpolation  (unchanged from v2)
# ---------------------------------------------------------------------------
...
class MultiRateSimulator:
    ...
    v3: accepts optional PackParams; uses BatteryPack when provided.
    """
    ...
    def __init__(
        ...
        if pp is not None:
            self.plant = BatteryPack(bp, tp, thp, pp)
            self.n_cells = pp.n_cells
        else:
            self.plant = BatteryPlant(bp, tp, thp)
            self.n_cells = 1

        # Optimizer-level components (pack-level, unchanged from v2)
    ...
    def run(
        ...
        # Pre-allocate storage (pack-level, same as v2)
        ...
        # Reference tracking (for metrics)
        ...
        # Cell-level arrays (v3 — only when multi-cell)
