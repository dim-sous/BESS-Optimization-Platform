...
The plant is a BatteryPack wrapping N BatteryPlant instances with
per-cell parameter variation and proportional active balancing.
"""
...
def main() -> None:
    ...
    logger.info("  SOC spread: +/-%.1f%%", pp.initial_soc_spread * 100)
    ...
    plot_results(results, bp, thp, pp, save_path=plot_path)
    ...
    if "cell_socs" in results:
        ...
        print(f"  Max temp spread:     {np.max(temp_spread):.2f} degC")
