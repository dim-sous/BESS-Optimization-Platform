"""Six-panel result visualisation for v4 electrical RC model.

Panel layout (3 x 2)
---------------------
  [0,0] SOC + cell spread              |  [0,1] SOH + cell spread
  [1,0] Temperature                    |  [1,1] Terminal voltage + OCV (v4 hero)
  [2,0] Power dispatch + P_reg + price |  [2,1] Cumulative profit breakdown
...
def plot_results(
    ...
    fig.suptitle(f"v4 Electrical RC Model ({n_cells} cells) — 24 h Simulation",
    ...
    # ── Terminal Voltage + OCV (hero) ────────────────────────────────────
    ax = axes[1, 1]
    vterm = sim.get("vterm_true")
    if vterm is not None:
        ax.plot(t_sim, vterm, color="tab:blue", lw=_LW, label="V_term")
        ocv = ocv_pack_numpy(sim["soc_true"], elp)
        ax.plot(t_sim, ocv, color="tab:green", lw=1.0, alpha=0.6,
                ls="--", label="OCV(SOC)")
        ax.axhline(elp.V_min_pack, color="red", lw=1.0, ls=":",
                   label=f"V_min = {elp.V_min_pack:.0f} V")
        ax.axhline(elp.V_max_pack, color="red", lw=1.0, ls=":")
    ax.set(ylabel="Voltage [V]")
    ax.set_title("Pack Terminal Voltage (v4 upgrade)")
    ax.legend(loc="upper right")
    ax.xaxis.set_minor_locator(AutoMinorLocator())
    ax.grid(True, alpha=0.25)
