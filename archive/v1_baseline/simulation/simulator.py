"""Multi-rate simulation coordinator with timing instrumentation.
...
def interpolate_ems_to_mpc(
    ...
    State references hold the end-of-hour target constant, letting
    the MPC decide its own intra-hour trajectory.
...
class MultiRateSimulator:
    ...
    def run(
        ...
        # Reference tracking (for metrics computation)
        ...
        for sim_step in range(N_sim_steps):
            ...
            if sim_step % steps_per_ems == 0:
                ...
                # endpoint to the new plan over n_blend_steps.
                if ems_hour > 0:
