"""Multi-rate simulation coordinator with thermal model and timing.

v2_thermal_model: 3-state (SOC, SOH, Temperature) dynamics.
...
def interpolate_ems_to_mpc(
    ...
    Both power and state references use **zero-order hold**.
    State references hold the end-of-hour target constant.
    """
    ...
    def run(
        ...
        for sim_step in range(N_sim_steps):
            ...
            if sim_step % steps_per_ems == 0:
                ...
                # Save old power endpoint for blending
                if ems_hour > 0:
            ...
            if sim_step % steps_per_mpc == 0:
                y_meas = self.plant.get_measurement()  # ndarray(2,)
